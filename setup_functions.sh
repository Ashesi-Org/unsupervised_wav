...
install_pytorch_and_other_packages() {
    ...
    # note we need a torch version less than 2.6 to prepare audio properly 
        ...
        echo "torch_cuda_suffix: $torch_cuda_suffix"
...
install_pykaldi() {
    ...
    if [ -d "$PYKALDI_ROOT" ]; then
        ...
        git clone https://github.com/Ashesi-Org/pykaldi.git "$PYKALDI_ROOT" #i will push them to ashesi github and use their link
        cd "$PYKALDI_ROOT"
    fi

    pip install --upgrade pip setuptools
    pip install numpy pyparsing ninja wheel cython 
    ...
    pip install . --no-build-isolation     # commented out the original code of --"python setup.py install" as it was causing errors 

    source "$VENV_PATH/bin/activate"
    pip uninstall tensorboardX #the installation of pykaldi changes the required version of tensorboard to successfully run other versions of the script
