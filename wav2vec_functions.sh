...
FAIRSEQ_ROOT="$DIR_PATH/fairseq" # the root directory of the fairseq repository
...
prepare_text() {
   ...
   if is_completed "$step_name"; then
    ...
    zsh "$FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/prepare_text.sh" ak $UNLABELLED_TEXT $TEXT_OUTPUT $MIN_PHONES G2P "$FASTTEXT_LIB_MODEL" 0.25
